...
export class QntdTempoAtrasPipe implements PipeTransform {
  ...
  transform(data: string): string {
    ...
    if(diffSegundos < 60){
      return `há ${Math.floor(diffSegundos)} segundos atrás`
    }
    else if(diffMinutos < 60){
      return `há ${Math.floor(diffMinutos)} minutos atrás`
    }
    else if(diffHoras < 60){
      return `há ${Math.floor(diffHoras)} horas atrás`
    }
    else{
      return `há ${Math.floor(diffDias)} dias atrás`
    }
  }
